...
const FOOT_BODY = { x: 0.62, y: 0.46, z: 1.0 };
...
function spearArm(length) {
  return shaft({ x: 0.34, y: -0.1, z: 0.35 }, length, 0.09, -0.62, MATERIALS.shaft);
}

/** A short sword, near vertical. */
function swordArm() {
  return shaft({ x: 0.36, y: 0.05, z: 0.5 }, 0.62, 0.11, 0.18, MATERIALS.blade);
}

/** A sabre: two segments, the upper one kicked over to suggest a curve. */
function sabreArm() {
  const lower = shaft({ x: 0.34, y: 0.02, z: 0.9 }, 0.4, 0.1, 0.25, MATERIALS.blade);
  const upper = shaft({ x: 0.34, y: 0.12, z: 1.28 }, 0.36, 0.09, 0.75, MATERIALS.blade);
  return [...lower, ...upper];
}

const HORSE_BODY = { x: 0.62, y: 1.7, z: 0.6 };
const LEG_HEIGHT = 0.62;

function horseman({ weapon }) {
  const legs = [[-0.24, -0.6], [0.24, -0.6], [-0.24, 0.6], [0.24, 0.6]]
    .flatMap(([x, y]) => cylinder({ x, y, z: 0 }, 0.12, LEG_HEIGHT, MATERIALS.horseDark, 4));
  const body = box({ x: 0, y: 0, z: LEG_HEIGHT }, HORSE_BODY, MATERIALS.horseHide);
  const seat = LEG_HEIGHT + HORSE_BODY.z;
  const rider = box({ x: 0, y: -0.12, z: seat }, { x: 0.46, y: 0.42, z: 0.62 }, MATERIALS.rider);
  const head = ball({ x: 0, y: -0.12, z: seat + 0.62 }, 0.26, MATERIALS.tanSkin);
  const armed = shift(weapon, { x: 0, y: -0.12 }).map((face) => ({
    material: face.material,
  ...
  return {
    ...
    plain: [
      ...mount,
      ...box({ x: 0, y: -0.12, z: seat }, { x: 0.46, y: 0.42, z: 0.86 }, MATERIALS.rider),
    ],
    speck: [mount[0]],
  };
}
...
const FORMATIONS = {
  IR0: () => ({
    places: swarm(14, 1.9, seededRandom(7)),
    figure: footSoldier({
  ...
  CR0: () => ({
    places: ranks(5, 2, 1.7, 2.4),
    figure: horseman({ weapon: sabreArm() }),
  }),
  CR1: () => ({
    places: wedge(4, 1.8, 2.3),
    figure: horseman({ weapon: shaft({ x: 0.34, y: 0, z: 0.4 }, 2.4, 0.09, 0.05, MATERIALS.shaft) }),
  }),
};
